...
from ..a2a_types.core import (
    Agent, Task, Message, Artifact, AgentId, TaskId,
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code != expected_status:
                logger.warning(f"Expected status {expected_status} but got {response.status_code} from {url}")
...
async def create_task(
    ...
    endpoint = f"{str(target_agent.endpoint).rstrip('/')}/tasks" # Standard RESTful endpoint
...
async def send_message(
    ...
    try:
        ...
        logger.error(f"Failed to send message to agent {target_agent.id} for task {task_id}: {e}")
        return None
